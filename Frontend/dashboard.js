...
function renderNav(user) {
  ...
  const tabsByRole = {
    admin: [["overview", "Overview"], ["members", "Members"], ["trainers", "Trainers"], ["payments", "Payments"], ["plans", "Plans"], ["reviews", "Reviews"], ["messages", "Messages"]],
...
function renderAdminReviews() {
  ...
  content.innerHTML = `
    <article class="data-card">
      <h3>Homepage Reviews</h3>
      ${tableMarkup(["Name", "Date", "Rating", "Feedback", "Actions"], reviews.slice().reverse().map((review) => `
        <tr>
          <td>${escapeHtml(review.name)}</td>
          <td>${escapeHtml(review.date)}</td>
          <td>${"★".repeat(review.rating)}</td>
          <td>${escapeHtml(review.feedback)}</td>
          <td><button class="mini-btn danger" data-delete-review="${review.id}">Delete</button></td>
        </tr>`))}
    </article>`;

  document.querySelectorAll("[data-delete-review]").forEach((button) => button.addEventListener("click", () => {
    writeStore(REVIEWS_KEY, readStore(REVIEWS_KEY, []).filter((review) => review.id !== button.dataset.deleteReview));
    renderAdminReviews();
  }));
}
